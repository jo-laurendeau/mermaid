...
- [2026-03-27] **CRITICAL: NEVER set `subGraphTitleTotalMargin = 0` for state diagrams.** This margin (default: `top: 10, bottom: 0` from `config.schema.yaml`) is the ONLY mechanism that prevents cluster titles from colliding with their child nodes. It works via 3 post-layout shifts in `dagre/index.js::recursiveRender()`: `clusterNode.y += margin` (L184), `regularNode.y += margin/2` (L224), `edgePoint.y += margin/2` (L253). Disabling it causes immediate collision. The correct approach for compaction is to reduce the schema default from `10` to `5` (minimum safe), NOT to zero it out.
- [2026-03-27] **Dagre spacing for state diagrams is ALREADY minimal.** `stateRenderer-v3-unified.ts` lines 74-75 set `nodeSpacing = 2` and `rankSpacing = 2`. The fallback chain in `dagre/index.js` correctly resolves these. Hardcoding `nodesep: 2` / `ranksep: 2` in the Dagre graph config is redundant and dangerous (bypasses the config pipeline).
- [2026-03-27] **`padding` on Dagre nodes ≠ title margin.** Node padding inflates the cluster bounding box equally on all 4 sides. Increasing it to 35-45px does NOT compensate for a disabled title margin — it makes clusters enormous while titles still collide. The title clearance is a separate post-layout mechanism.
